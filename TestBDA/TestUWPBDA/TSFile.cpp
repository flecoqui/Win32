...
int TSFile::Open(const TCHAR* ts_file)
{
	...
	m_file_handle = CreateFile2(ts_file, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, OPEN_ALWAYS, &pCreateExParams);
...
int TSFile::Create(const TCHAR* ts_file)
{
	CREATEFILE2_EXTENDED_PARAMETERS pCreateExParams = { 0 };
	pCreateExParams.dwSize = sizeof(CREATEFILE2_EXTENDED_PARAMETERS);
	pCreateExParams.dwFileAttributes = FILE_ATTRIBUTE_NORMAL;
	pCreateExParams.lpSecurityAttributes = NULL;
	pCreateExParams.hTemplateFile = NULL;
	m_file_handle = CreateFile2(ts_file, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, OPEN_ALWAYS, &pCreateExParams);
	int error = GetLastError();
...
{	
	LARGE_INTEGER dwPtr ;
	dwPtr.QuadPart = 0;
	bool crt = SetFilePointerEx(m_file_handle, dwPtr, &dwPtr, FILE_BEGIN);
	if (crt == false) // Test for failure
...
{	 
	...
	//		m_buffer_handle = GlobalAlloc(GHND, m_buffer_size);
		//	if(m_buffer_handle)
	//		{
				m_buffer_pointer = new BYTE[m_buffer_size];
//				m_buffer_pointer = (LPBYTE) GlobalLock(m_buffer_handle);
				if(m_buffer_pointer)
				{
					Read(0);
...
int TSFile::Unload(void)
{
	if(m_buffer_handle)
	{
		if(m_buffer_pointer!=NULL)
			delete m_buffer_pointer;
		m_buffer_pointer = NULL;
...
0,0, TEXT("ITU-T | ISO/IEC reserved"),
1,1, TEXT("ISO/IEC 11172-2 Video"),
2,2, TEXT("ITU-T Rec. H.262 | ISO/IEC 13818-2 Video or ISO/IEC 11172-2 constrained parameter video stream"),
3,3, TEXT("ISO/IEC 11172-3 Audio"),
4,4, TEXT("ISO/IEC 13818-3 Audio"),
5,5, TEXT("ITU-T Rec. H.222.0 | ISO/IEC 13818-1 private_sections"),
6,6, TEXT("ITU-T Rec. H.222.0 | ISO/IEC 13818-1 PES packets containing private data"),
7,7, TEXT("ISO/IEC 13522 MHEG"),
8,8, TEXT("Annex A – DSM CC"),
9,9, TEXT("ITU-T Rec. H.222.1"),
10,10, TEXT("ISO/IEC 13818-6 type A"),
11,11, TEXT("ISO/IEC 13818-6 type B"),
12,12, TEXT("ISO/IEC 13818-6 type C"),
13,13, TEXT("ISO/IEC 13818-6 type D"),
14,14, TEXT("ISO/IEC 13818-1 auxiliary"),
15,127, TEXT("ITU-T Rec. H.222.0 | ISO/IEC 13818-1 reserved"),
128,255,TEXT("User private")};
